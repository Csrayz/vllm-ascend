...
class AscendDSACPMetadataBuilder(AttentionMetadataBuilder[AscendDSAMetadata]):
    ...
    def __init__(
        ...
        if self.speculative_config:
            ...
            self.spec_sas_metadata = [
                torch.zeros(SAS_METADATA_SIZE, dtype=torch.int32, device=self.device)
                for _ in range(spec_token_num)
            ]
    ...
    def build_req_metadata_for_drafting(
        ...
        # .clone() removed — spec_local_query_start_loc / spec_local_seq_lens
        # are pre-allocated per-step buffers with stable addresses for graph mode.
        ...
        # Cache sas_metadata in per-step buffer for address stability.
